...
def resume_preview(request, template_id):
    ...
    template_map = {
        1: 'resumes/resume_samira.html',
        2: 'resumes/resume_isabel.html',
        3: 'resumes/resume_laurice.html',
        4: 'resumes/resume_olivia.html',
        5: 'resumes/resume_jacqueline.html',
        6: 'resumes/resume_pragya.html',
        11: 'resumes/resume_lorna.html',
        12: 'resumes/resume_hannah.html',
        13: 'resumes/resume_seema.html',
        14: 'resumes/resume_emaa.html',
    }
    template_file = template_map.get(selected_template.id, 'resume_preview.html')
    ...
    if request.method == 'POST':
        try:
            ...
            if "429" in error_message or "Quota" in error_message:
                return JsonResponse({
                    'status': 'error', 
                    'message': 'AI is busy. Please wait 60 seconds and try again.'
                }, status=429)
...
def select_cover_letter_template(request):
    ...
    return render(request, 'select_cover_letter_template.html', {'cl_templates': cl_templates})

@login_required
def cover_letter_builder(request, template_id):
    template = get_object_or_404(CoverLetterTemplate, id=template_id)
    return render(request, 'cover_letter_builder.html', {'template': template})

def cl_preview_blank(request, template_id):
    template = get_object_or_404(CoverLetterTemplate, id=template_id)
    return render(request, template.file_path, {'template': template})
...
def ai_enhance_text(request):
    if request.method == "POST":
        try:
            ...
            # ✅ Use single unified SDK configured at top
            model = genai.GenerativeModel('gemini-2.5-flash')
            response = model.generate_content(prompt)
            return JsonResponse({"success": True, "enhanced_text": response.text.strip()})
        except Exception as e:
